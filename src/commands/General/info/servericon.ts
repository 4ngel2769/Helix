...
export class ServericonCommand extends ModuleCommand<GeneralModule> {
  ...
  public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
    ...
    try {
      ...
      if (!guild.icon) return interaction.editReply('This server has no icon.');

      return interaction.editReply({ embeds: [this.buildServerIconEmbed(guild)] });
    } catch (error) {
  ...
  public override async messageRun(message: Message) {
    try {
      ...
      if (!guild.icon) return message.reply('This server has no icon.');

      return message.reply({ embeds: [this.buildServerIconEmbed(guild)] });
    } catch (error) {
      this.container.logger.error('Error in servericon:', error);
  ...
  private buildServerIconEmbed(guild: Guild) {
